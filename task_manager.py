...
def reg_user():
    """
    This function checks if the user logged in is an admin
    If so, print menu option title as well as the input for new username
    If that username already exists, print an error message and ask the user for username input again
    When a correct new username is inputted, ask for password and password confirmation
    """
...
"""
DOCUMENTATION:
https://www.programiz.com/python-programming/datetime/strptime. Here I learnt how to use the strptime method to create a
date object. I needed this to be able to use the string date from the text file to create an object so I can then 
compare it to the current date to see if any tasks are overdue.

I also used https://realpython.com/python-counter/ to learn about the existence of a library called "collections" which
has a function called "counter". This function is used to count the frequency of a repeated item. I used this to count
how many tasks are there assigned to each user and create a dictionary that has the user name as keys and the counter as
values.

https://pythonhow.com/how/check-if-a-text-file-is-empty/#:~:text=txt%20is%20empty%20if%20os,%22File%20is%20empty!%22. I
have used this page to learn how to check if a text file is empty or not. I have learnt that you can use the os module,
specifically the .path.getsize() methods. If this is equal to 0, the document is empty.
"""
